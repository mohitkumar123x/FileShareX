    ...
    const handlePurchase = async (plan) => {
        ...
        try {
            ...
            if (response.data.isMock) {
                // Mock payment - skip Razorpay and directly verify
                try {
                    ...
                    if (verifyResponse.data.success) {
                        ...
                        setMessage(`Payment successful! ${plan.name} plan activated (Mock Mode).`);
            ...
            setMessage(error.response?.data?.message || "Failed to initiate payment. Please try again later.");
            setMessageType("error");
        } finally {
    ...
    return (
        <DashboardLayout activeMenu="Subscription">
            <div className="p-6">
                ...
                <p className="text-gray-600 mb-6 dark:text-zinc-400">Choose a plan that works for you</p>

                {message && (
                    <div className={`mb-6 p-4 rounded-lg flex items-center gap-3 ${
                        messageType === 'error' ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300' :
                            messageType === 'success' ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300' :
                ...
                <div className="flex flex-col md:flex-row gap-6 mb-8">
                    <div className="bg-blue-50 p-6 rounded-lg dark:bg-blue-500/10 dark:border dark:border-blue-500/20">
                        <div className="flex items-center gap-2 mb-4">
                            <CreditCard className="text-purple-500" />
                            <h2 className="text-lg font-medium dark:text-zinc-100">Current Credits: <span className="font-bold text-purple-500">{credits}</span></h2>
                        </div>
                        <p className="text-sm text-gray-600 mt-2 dark:text-zinc-400">
                    ...
                    {plans.map((plan) => (
                        <div
                            ...
                            className={`border rounded-xl p-6 ${
                                plan.recommended
                                    ? 'border-purple-200 bg-purple-50 shadow-md dark:border-purple-500/30 dark:bg-purple-500/10'
                        ...
                        >
                            ...
                            <div className="mt-2 mb-4">
                                <span className="text-3xl font-bold dark:text-zinc-100">&#8377;{plan.price}</span>
                                <span className="text-gray-500 dark:text-zinc-400"> for {plan.credits} credits</span>
                            </div>
                            ...
                            <ul className="space-y-3 mb-6">
                                {plan.features.map((feature, index) => (
                                    <li key={index} className="flex items-start">
                                        ...
                                        <span className="dark:text-zinc-300">{feature}</span>
                                    </li>
                                ))}
                            </ul>
                            ...
                            <button
                                ...
                                className={`w-full py-2 rounded-md font-medium transition-colors ${
                                    plan.recommended
                                        ...
                                        : 'bg-white border border-purple-500 text-purple-500 hover:bg-purple-50 dark:bg-zinc-900 dark:text-purple-300 dark:border-purple-400/70 dark:hover:bg-purple-500/10'
                ...
                <div className="mt-8 bg-gray-50 p-4 rounded-lg border border-gray-200 dark:bg-zinc-900 dark:border-zinc-800">
                    <h3 className="font-medium mb-2 dark:text-zinc-100">How credits work</h3>
